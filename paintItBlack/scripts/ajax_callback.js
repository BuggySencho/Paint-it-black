...
{
  let iansThing = JSON.stringify(data); // object naar string
  console.log(iansThing);
let url = controlScript + "?put=" + iansThing;
makeAjaxCall(url, "GET");
}

function ServerGetJson()
{
//  url = controlScript + "?put=" + iansThing;
makeAjaxCall(jsonFile, "POST"). then (readJson); // vraag data aan server, als ontvangen dan verwerken
}
